...
#' @export
f.plot.mxnt.preds <- function(mmp.spl, mtp.spl, pred.nm=""){
  ...
  comb.plots <- combn(nlayers(mtp.spl[[1]]$continuous[[2]]), 2)
  ...
  for(i in 1:length(mtp.spl)){
    ...
    for(l in 1:length(mods.thrshld$binary)){
      ...
      pdf(paste(thrshld.path, paste0("Mod.diff.bin", pred.nm, ".", thr.crt, ".pdf"), sep='/'),
          width = 20, height = 10)
      ...
      for(j in 1:ncol(comb.plots)){ #ncol(comb.plots)
        ...
        r.dif <- overlay(mods.thrshld$binary[[l]][[comb.plots[1,j]]], mods.thrshld$binary[[l]][[comb.plots[2,j]]], fun=function(r1,r2) {r1-r2})
        ...
        plot(r.dif, breaks= c(-1, -.33, .33, 1), col=c("blue", "white", "red"),# col=c("blue", "blue","white", "red", "red"), # smallplot=c(0.01, .81, 0.01, .99),
             main= main.nms, legend=FALSE)
        if(j==1){
          title(paste("Threshold criteria:", thr.crt), line = 2, outer = T, cex.main=2)
        }
        plot(r.dif,  legend.only=TRUE, smallplot=c(.78, .79, .2, .8),  #horiz=T,#  c(.79, .80, .2, .8)
...
f.plot.mxnt.preds.mscn <- function(mmp.spl, mtp.spl, basemap){
  ...
  comb.plots <- combn(nlayers(mtp.spl[[1]]$mxnt.preds$binary[[1]]), 2)
  ...
  for(sp in 1:length(mtp.spl)){ # species
    ...
    for(sc in names(mtp.spl[[sp]])){ # climatic scenario
      ...
      for(l in names(mods.thrshld$binary)){ # threshold criteria
        ...
        pdf(paste(thrshld.path, paste0("Mod.diff.bin.", sc, ".", thr.crt, ".pdf"), sep='/'),
            width = 20, height = 10)
        # par(mfrow=c(3,5), mar=c(2,1,2,1), oma = c(1, 1, 4, 1))
        par(mfrow=c(3,5), mar=c(2,4,2,5), oma = c(3.5, 0, 5.5, 2))
        ...
        for(j in 1:ncol(comb.plots)){ #ncol(comb.plots)
          ...
          # r.dif, breaks= c(-1, -.33, .33, 1), col=c("blue", "white", "red")
          par(mar=c(2,4,2,4.7)) # par(mar=c(2,4,2,5))
          plot(mods.thrshld$binary[[l]][[comb.plots[1,j]]], breaks= c(0, .5, 1), col=c("white", "gray90"),
          ...
          if(j==1){
            ...
            title(paste0("Climatic Scenario: ", sub("mxnt.pred.", "", sub("mxnt.preds", "bioclim", sc1))), line = 1.5, outer = T, cex.main=2)
            title(paste0("Threshold Criterion: ", thr.CRT), line = -.5, outer = T, cex.main=2)
          }
          plot(basemap, border="gray50", add= T)
          plot(r.dif, breaks= c(-1, -.33, .33, 1), col=c("blue", rgb(0,0,0,0), "red"),
               legend=FALSE, add=T) # main= main.nms,

          par(mar=c(2,1,2,6)) # par(mar=c(2,3,2,6))
          plot(r.dif,  legend.only=TRUE, legend.width=1.75, legend.shrink=.75, #smallplot=c(.78, .79, .2, .8),  #horiz=T,#  c(.79, .80, .2, .8)
